...
wm.Weltmeister = ig.Class.extend({	
	...
	loadResponse: function( data ) {
		...
		for( var i=0; i < data.layer.length; i++ ) {
			...
			newLayer.startheight = ld.startheight;
	...
	updateLayerSettings: function( ) {
		...
		$('#layerStartheight').val( this.activeLayer.startheight );
	},
	...
	saveLayerSettings: function() {
		...
		if( isCollision ) {
			...
			this.activeLayer.distance = Math.floor($('#layerDistance').val());
			this.activeLayer.startheight = Math.floor($('#layerStartheight').val());
	...
	toggleCollisionLayer: function( ev ) {
		var isCollision = $('#layerIsCollision').attr('checked') ? true : false;
		$('#layerLinkWithCollision,#layerDistance,#layerStartheight,#layerPreRender,#layerRepeat,#layerName,#layerTileset')
	...
	keyup: function( action ) {
		...
		if( action == 'scrollup' ) {
			ig.system.imageYpos += 600;
			if (ig.system.imageYpos > 0){
				ig.system.imageYpos = 0;
			}
		}
